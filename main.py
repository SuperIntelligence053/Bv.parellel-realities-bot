...
def hf_generate_image(hf_token: str, prompt: str, out_path: Path):
    ...
    payload = {
        "inputs": prompt,
        "parameters": {"width": 1024, "height": 1792, "guidance_scale": 7.0, "num_inference_steps": 30},
    }
...
def build_video(images, audio_mp3: Path, captions_lines, out_mp4: Path):
    ...
    for line in cap:
        ...
        draw_filters.append(
            f"drawtext=text='{safe}':x=(w-text_w)/2:y=h*0.78:fontsize=56:fontcolor=white:borderw=3:"
...
def main():
    ...
    images = []
    for i, p in enumerate(story["image_prompts"], start=1):
    ...
    resp = upload_video(yt, out_mp4, story["title"], story["description"], story.get("tags", []))
    video_id = resp.get("id")
    print("✅ Uploaded:", video_id)

    state["day"] = day + 1
    state.setdefault("uploads", []).append({"day": day + 1, "video_id": video_id, "title": story["title"]})
    save_state(state)
